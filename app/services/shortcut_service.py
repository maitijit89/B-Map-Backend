...
from app.db.models import UserShortcut
...
class ShortcutService:
    ...
    async def create_or_update_shortcut(
        ...
        """
        Create or update shortcut (Home / Work).
        """
        location = {
            "type": "Point",
            "coordinates": [lng, lat]
        ...
        # Check if already exists for this user/name
    ...
    async def get_shortcuts(self, user_id: str, current_location_str: str = None, db: Any = None) -> List[Dict[str, Any]]:
        """
        Retrieve shortcuts for a user and calculate live travel ETAs if current_location is provided.
        """
        shortcuts_list = []
        if db is not None:
        ...
        # Fallback default shortcuts if none in DB (for user onboarding / testing)
