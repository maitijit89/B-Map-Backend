...
class PlaceService:
    ...
    async def autocomplete(self, input: str, lat: Optional[float] = None, lng: Optional[float] = None):
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.get(url, params=params)
                data = response.json()
                await cache.set(cache_key, data, expire=1800)
                return data
        except Exception:
    ...
    async def reverse_geocode(self, lat: float, lng: float):
        ...
        # Try TomTom Reverse Geocoding API if available
        from app.shared.integrations.tomtom_service import TomTomService
        tomtom_service = TomTomService()
        if tomtom_service.is_configured():
            tomtom_res = await tomtom_service.reverse_geocode(lat, lng)
            if tomtom_res:
                res_data = {
                    "results": [
                        {
                            "formatted_address": tomtom_res.get("formatted_address"),
                            "geometry": {
                                "location": {"lat": lat, "lng": lng},
                                "location_type": "GEOMETRIC_CENTER"
                            },
                            "place_id": f"tomtom_{lat}_{lng}",
                            "types": ["street_address"],
                ...
                await cache.set(cache_key, res_data, expire=86400)
                return res_data
